...
def Final_Detecting_Object():
    ...
    def intermediate_filter():
        ...
        start_time_200 = time.ticks_ms() # Запоминаем время начала
        object_stays_detected = True  # Флаг, указывающий, остаётся ли объект в кадре

        while time.ticks_ms() - start_time_200 < 200:  # Проверяем в течение 200 мс
            # Захватываем новое изображение для анализа
            img = sensor.snapshot()
            ...
            if max_blob_:
                ...
                # Устанавливаем флаг
                object_stays_detected = True
                #print(object_stays_detected)
                ...
                # Выходим из цикла
                if time.ticks_ms() - start_time_200 <= 180:
                    break

            else:

                # Устанавливаем флаг
                object_stays_detected = False
                #print(object_stays_detected)

                if time.ticks_ms() - start_time_200 <= 180:
                    break

        return object_stays_detected


    start_time_1000 = time.ticks_ms() # Запоминаем время начала
    counter_object_detected_false = 0 # Счётчик колученых фрагметнов с false
    counter_object_detected_true = 0 # Счётчик колученых фрагметнов с true

    while time.ticks_ms() - start_time_1000 < 1000:

        object_detected = intermediate_filter()

        for n in range (5):

            if object_detected == False and counter_object_detected_true == 0:
                counter_object_detected_false += 1
                print (counter_object_detected_false)

            elif object_detected == True and counter_object_detected_false == 1:
                counter_object_detected_false += 1
                print (counter_object_detected_true)

        if counter_object_detected_false == 5:
            ...
            print("Сигнал подан, объект не обнаружен в течение 200 миллисекунд.")

        elif counter_object_detected_true == 5:

            print("Объект обнаружен.")

        else:

            print("Ошибка обноружения!!!")
