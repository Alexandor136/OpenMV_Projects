...
def Final_Detecting_Object():
    ...
    while True:
        ...
        if time.time() - start_time >= 1:  # Если прошло 1 секунда
            # Вычисляем отношение
            if total_frames > 0:
                # Отношение обнаруженных объектов к общему количеству кадров
                ratio = count_detected_object / total_frames
                print(f"Отношение кадров с объектами: {ratio:.2f} (Обнаружено: {count_detected_object}, Не обнаружено: {count_no_object})")
                if ratio >= 0.4:
                    control_relay(True)
                ...
                print("Не было кадров за 1 секунду.")
                control_relay(False)
                flag_exit_fun = 1
