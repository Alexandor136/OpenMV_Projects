...
def Final_Detecting_Object():
    ...
    while (time.time() - start_time) < 5:
        ...
        if result == 1:
            signal = 1
            if result == 0: # Проверака на проподание объекта
                signal = 0 # Объект обноружен нет разрешающего сигнала
                #break

        elif result == 0:
            signal = 0
            if result == 1: # Проверака на появление объекта
                signal = 1 # Объект обноружен нет разрешающего сигнала
                #break

        if (time.time() - start_time) == 0.6 and signal == 0:
        ...
        if (time.time() - start_time) > 0.6 and (time.time() - start_time) < 4.9:
            time.sleep(0.01)
