...
def Set_Gain_Min(img, thresholds, max_blob_area_limit, max_blob_area, max_blob,
                 min_area_threshold, max_gain):
    ...
    while True:
        ...
        max_blob_, max_blob_area_, cnt_ = Detecting_Object(img, thresholds, max_blob_area_limit,
                                                           max_blob_area, max_blob)
        # Если объект не обноружен
        if max_blob_ == None and gain < max_gain:

            #Увеличиваем текущее значение
            gain += 0.1
            sensor.set_auto_gain(False, gain_db=gain)
            sensor.snapshot()
            print (gain,'fun_Set_Gain_Min')

        elif max_blob_ == None and gain >= max_gain:
            print('erorr: Настройка невозможна!')

        elif max_blob_ != None and gain !=0:
            print('Минимальный уровень усиления найден!', gain)
            return gain, max_blob_
...
def calibration(img, thresholds, max_blob_area_limit, max_blob_area, max_blob, min_area_threshold, gain_type):
    if gain_type == 'max':
        gain = Set_Gain_Max(img, thresholds, max_blob_area_limit, max_blob_area, max_blob, min_area_threshold)
    else:
        gain = Set_Gain_Min(img, thresholds, max_blob_area_limit, max_blob_area, max_blob, min_area_threshold, max_gain)
    sensor.snapshot()
    return gain
...
# Начальное значение усиления
gain_global = 24
sensor.set_auto_gain(False, gain_db=gain_global)
...
while True:
    ...
    if button.value() == 0:
        ...
        if check_button():
            ...
            img = setup_mode()
            max_gain = calibration(img, thresholds, max_blob_area_limit, max_blob_area, max_blob, min_area_threshold, 'max')
            if max_gain is not None:
                ...
                while True:
                    ...
                    if button.value() == 0:
                        if check_button():
                            print('Калибровка Min_Gain')
                            min_gain = Set_Gain_Min(img, thresholds, max_blob_area_limit,
                                         max_blob_area, max_blob,
                                         min_area_threshold, max_gain)
                            if min_gain != None:
                                print(min_gain, 'main')
