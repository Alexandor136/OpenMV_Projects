...
def Init_Device():
    # Инициализируем кнопку на контакте P4, P6, P5 как вход
...
def Final_Detecting_Object():
    ...
    result = 1  # Начальное значение сигнала
    img = switch_mod_img(img_binary_flag, thresholds)

    while (time.time() - start_time) < 5:
        result = check_object_in_frame()

        # Если объект не обнаружен в течение 1 секунды
        if (time.time() - start_time) >= 1 and result == 0:
            control_relay(True)  # Подать сигнал на реле
            time.sleep(1)  # Задержка для реле
            control_relay(False)
            print('relay')
        ...
        # Если объект был обнаружен в течение первой секунды
        if (time.time() - start_time) < 1 and result == 1:
            #time.sleep(0.1)  # Ждем немного, чтобы избежать "дребезга" сигнала
            continue  # Продолжаем проверять наличие объекта

        # Если объект был обнаружен в течение первой секунды и продолжаем проверять
        if (time.time() - start_time) >= 1 and result == 1:
            check_start_time = time.time()  # Запоминаем время начала проверки
            while (time.time() - check_start_time) < 4:  # Проверяем в течение 4 секунд
                result = check_object_in_frame()  # Проверяем наличие объекта
                if result == 0:  # Если объект пропал
                    control_relay(True)  # Подать сигнал на реле
                    time.sleep(1)  # Задержка для реле
                    control_relay(False)
                    print('relay')
                    break  # Выходим из цикла, так как объект пропал

            # Проверка завершена, если объект все еще присутствует
            if result == 1:
                ...
                while True:
                    # Если нажата кнопка или поступили сигнал от термопласта
